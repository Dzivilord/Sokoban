...
def renderMap(board):
    width = len(board[0])
    height = len(board)
    indent = (640 - width * 33) / 2.0  
    for i in range(height):
        for j in range(width):
            screen.blit(space, (j * 33 + indent, i * 33 + 200))
            if board[i][j] == '#':
                screen.blit(wall, (j * 33 + indent, i * 33 + 200))
            elif board[i][j] == '$':
                screen.blit(stone, (j * 33 + indent, i * 33 + 200))
            elif board[i][j] == '.':
                screen.blit(checkPoint, (j * 33 + indent, i * 33 + 200))
            elif board[i][j] == '@':
                screen.blit(player, (j * 33 + indent, i * 33 + 200))
